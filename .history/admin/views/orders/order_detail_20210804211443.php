...
if(isset($_GET['action'])){
    ...
    $con->query($query);
    // sau khi update xong thì điều hướng về 
    header("location: ?option=order_detail&id=".$_GET['id']);
}
// bắt lại thông tin update đơn hàng 
...
<?php 
$id = $_GET['id'];
// lấy thông tin người đặt hàng và người nhận hàng và phương thức thanh toán 
...
<!-- Tiếp theo ta có mối liên hệ giữa bảng orders orderdetail và products  -->
<?php 
$query = "select b.*,c.name,c.image 
from orders a join orderdetail b on a.id = b.order_id join products c on b.product_id = c.product_id where a.id =".$order['id'];
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><img src="../upload_images/<?php echo $item['image'];?>" alt="" height="100" width="100"></td>
                <td>
                    <!-- update số lượng ở đây là update cho số lượng bảng orderdetail phải biết đơn hàng nào và mã sản phẩm là gì  -->
